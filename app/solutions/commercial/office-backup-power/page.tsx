...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const override = getAdminSeoOverride(content?.seoOverrides, '/solutions/commercial/office-backup-power');
  const seo = resolveStaticSeo('/solutions/commercial/office-backup-power', override);
...
export default async function OfficeBackupPowerPage({ searchParams }: Props) {
  ...
  const [completeSystems, inverters, batteries] = await Promise.all([
    getProductsForCategoryCode('Complete Systems OBP'),
    ...
    <main className="w-full bg-white flex flex-col">
      <div className="w-full px-6 md:px-20 py-4 border-b border-zinc-100">
        <div className="max-w-[1280px] mx-auto flex items-center gap-2 flex-wrap">
          ...
          <span className="text-zinc-400">/</span>
          <span className="text-zinc-500 font-['Onest'] text-sm font-medium">Office Backup Power</span>
        </div>
      </div>

      <div className="w-full bg-stone-50 px-6 md:px-20 breadcrumb-hero-shell flex flex-col items-center gap-3 text-center">
        <h1 className="breadcrumb-title-lock">Office Backup Power</h1>
        <p className="breadcrumb-description-lock max-w-[700px]">
          Explore complete office backup bundles with inverter and battery options to keep business operations uninterrupted.
        </p>
      </div>

      <div className="w-full px-6 md:px-20 py-10">
        <SolutionProductTabs
          basePath="/solutions/commercial/office-backup-power"
          tabs={tabs}
          activeTab={sp.tab}
          emptyMessage="No products available in this Office Backup Power tab yet."
        />
      </div>
    </main>
  );
