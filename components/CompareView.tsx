  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="relative">
        <select
          className="w-full h-12 px-4 pr-10 rounded-lg border border-zinc-300 bg-white text-sm font-['Onest'] text-zinc-700 appearance-none focus:outline-none focus:ring-2 focus:ring-sky-600"
          value={value?.slug ?? ''}
          onChange={(e) => {
...
export default function CompareView({ products }: Props) {
          ...
          values: selected.map((p) => {
            ...
            return (
              <ul className="flex flex-col gap-1">
                {features.map((f, idx) => (
                  <li key={`${f}-${idx}`} className="flex items-center gap-1.5 text-sm font-['Onest'] text-zinc-700">
            ...
            return (
              <div className="flex flex-wrap gap-1.5">
                {apps.map((a, idx) => (
                  <span
                    key={`${a}-${idx}`}
                    className="px-2.5 py-1 rounded-full bg-sky-50 text-sky-700 text-xs font-medium font-['Onest']"
                  >
                    {a}
                  </span>
                ))}
  ...
  return (
    <div className="w-full flex flex-col">
      {/* Hero — matches site-wide breadcrumb style */}
      <div className="w-full bg-stone-50 px-6 md:px-20 breadcrumb-hero-shell flex flex-col items-center gap-4 text-center">
        <h1 className="breadcrumb-title-lock leading-tight">Compare Up to 2 Products</h1>
        ...
        <div className="flex flex-col gap-6">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-sky-800 text-white">
                  <th className="text-left px-6 py-4 font-semibold font-['Onest'] w-44">Specification</th>
                  {selected.map((p, idx) => (
                    <th
                      key={p!.id}
              ...
              <tbody>
                {specRows.map((row, i) => (
                  <tr key={row.label} className={i % 2 === 0 ? 'bg-white' : 'bg-zinc-50'}>
                    <td className="px-6 py-4 text-zinc-500 font-medium font-['Onest'] align-top">
                      {row.label}
                    </td>
                    {row.values.map((val, j) => (
                      <td
                        key={j}
                        className={`px-6 py-4 text-zinc-800 font-['Onest'] align-top ${j > 0 ? 'border-l border-zinc-200' : ''}`}
                      >
                        {val}
                      </td>
                    ))}
              ...
              <tfoot>
                <tr className="bg-white border-t border-zinc-200">
                  <td className="px-6 py-4 text-zinc-500 font-medium font-['Onest']">Action</td>
                  {selected.map((p, idx) => (
                    <td key={p!.id} className={`px-6 py-4 ${idx > 0 ? 'border-l border-zinc-200' : ''}`}>
                      <a
                        ...
                        className="inline-flex px-6 py-2.5 bg-sky-700 hover:bg-sky-800 text-white text-sm font-semibold font-['Onest'] rounded-full transition-colors"
                      >
                        Buy Now
                      </a>
                    </td>
          ...
          <div className="md:hidden flex flex-col gap-4">
            ...
            {specRows.map((row, i) => (
              <div key={row.label} className={`rounded-xl border border-zinc-200 p-3 ${i % 2 === 0 ? 'bg-white' : 'bg-zinc-50'}`}>
                <p className="text-zinc-500 text-[11px] font-semibold font-['Onest'] uppercase tracking-wide mb-2">{row.label}</p>
                <div className="grid grid-cols-2 gap-3">
                  {row.values.map((val, j) => (
                    <div key={j} className="text-zinc-800 text-base md:text-lg font-['Onest'] break-words">
                      {val}
                    </div>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {selected.map((p) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="h-11 inline-flex items-center justify-center px-4 bg-sky-700 hover:bg-sky-800 text-white text-sm font-semibold font-['Onest'] rounded-full transition-colors"
                >
                  Buy Now
                </a>
              ))}
          ...
          <div className="flex items-center justify-center">
            <button
              ...
              className="w-full md:w-auto h-11 md:h-auto px-8 py-3 border border-sky-700 text-sky-700 hover:bg-sky-50 text-sm font-semibold font-['Onest'] rounded-full transition-colors"
            >
              Clear Comparison
            </button>
          </div>
