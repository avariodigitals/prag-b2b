...
export default function TechResourcesView({ products, selectedSlug }: Props) {
  ...
  return (
    <div className="w-full px-6 md:px-20 py-10 md:py-16">
      <div className="max-w-[1280px] mx-auto flex flex-col gap-10">
      {/* Product selector */}
      <div className="flex flex-col gap-5">
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 bg-sky-700 rounded-sm" />
          <span className="section-kicker">Select Product</span>
        </div>
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
          ...
          <div className="relative w-full md:w-[420px]">
            <select value={slug} onChange={handleSelect}
              className="w-full h-12 pl-4 pr-10 bg-white rounded-xl border-2 border-sky-700 text-zinc-700 text-sm font-['Onest'] appearance-none outline-none focus:border-sky-800">
      ...
      {selectedProduct
        ...
        : <p className="text-zinc-400 text-lg md:text-xl font-['Onest'] text-center py-10">Select a product above to view available documents.</p>
      }
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2">
        ...
        {!loading && (
          <span className="text-zinc-500 text-base md:text-lg font-['Onest']">
      ...
      {!loading && docs.length === 0 && (
        <p className="text-zinc-400 text-lg md:text-xl font-['Onest'] py-4">No documents available for this product.</p>
      )}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {doc.file_type && <span className="text-zinc-500 text-base md:text-lg font-['Onest']">{doc.file_type.toUpperCase()}</span>}
                    {doc.file_size && <><span className="text-zinc-300">•</span><span className="text-zinc-500 text-base md:text-lg font-['Onest']">{doc.file_size}</span></>}
                    {doc.pages && <><span className="text-zinc-300">•</span><span className="text-zinc-500 text-base md:text-lg font-['Onest']">{doc.pages} pages</span></>}
                  </div>
                </div>
              </div>
              <a href={doc.file_url} target="_blank" rel="noopener noreferrer" download
                className="flex items-center justify-center gap-2 px-5 py-2.5 bg-sky-700 hover:bg-sky-800 text-white text-sm font-semibold font-['Onest'] rounded-xl transition-colors w-full md:w-auto shrink-0">
