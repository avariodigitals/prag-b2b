    ...
    <div className="rounded-2xl flex flex-col overflow-hidden border border-zinc-200">
      ...
      <div className="px-6 py-6 bg-white flex flex-col gap-4 flex-1">
        <div className="flex items-center gap-3 flex-wrap">
          {cat && (
            <span className="px-3 py-1 bg-sky-700 rounded-full text-white text-xs font-semibold font-['Space_Grotesk']">
              {cat.name}
            </span>
          )}
          {secondaryCat && (
            <span className="text-zinc-600 text-sm font-['Space_Grotesk']">{secondaryCat.name}</span>
          )}
          {!secondaryCat && (
            <span className="text-zinc-500 text-sm font-['Space_Grotesk']" suppressHydrationWarning>{readTime(post)}</span>
          )}
        </div>
        <h3
          className="text-zinc-900 text-xl font-semibold font-['Onest'] line-clamp-2"
          dangerouslySetInnerHTML={{ __html: sanitize(post.title.rendered) }}
        />
        <p className="text-neutral-600 text-sm font-['Space_Grotesk'] line-clamp-2 flex-1">
          {stripHtml(post.excerpt.rendered)}
        </p>
        <span className="text-sky-700 text-sm font-['Space_Grotesk']" suppressHydrationWarning>
          {postDate(post)}
        </span>
      </div>
    </div>
    ...
    <section className="w-full bg-white py-16 px-6 md:px-20">
      ...
      <div className="flex items-start justify-between mb-10">
        ...
        <Link
          ...
          className="hidden md:flex items-center gap-2 text-zinc-700 text-sm font-['Space_Grotesk'] hover:text-sky-700 transition-colors mt-4 shrink-0"
        >
          View all articles →
        </Link>
      </div>
      ...
      {posts.length > 0 ? (
        ...
        <p className="text-zinc-400 text-center py-10 font-['Space_Grotesk']">No articles found.</p>
      )}
      ...
      <div className="flex md:hidden justify-center mt-8">
        <Link
          href="/knowledge-center"
          className="flex items-center gap-2 text-zinc-700 text-sm font-['Space_Grotesk'] hover:text-sky-700 transition-colors"
        >
          View all articles →
        </Link>
      </div>
