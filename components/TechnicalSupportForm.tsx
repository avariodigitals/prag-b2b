...
async function submitTechnicalSupport(data: {
  name: string; email: string; phone?: string; company?: string; enquiry_type?: string; message: string;
...
function validateSupportForm(form: typeof EMPTY_FORM): string | null {
  ...
  if (form.enquiry_type && !ALLOWED_SUPPORT.has(form.enquiry_type)) return 'Select a valid support type.';
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-base font-bold font-['Space_Grotesk']">
          {toast.type === 'success' ? 'Ticket Submitted!' : 'Submission Failed'}
        </p>
...
export default function TechnicalSupportForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSending(true);
    const result = await submitTechnicalSupport(form);
    setSending(false);
    if (result.success) {
      setForm(EMPTY_FORM);
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          <label className={labelCls}>Describe your issue *</label>
          <textarea
            required
            value={form.message}
